  ...
  try {
    ...
    if (!adsError && ads && ads.length > 0) {
      adsContainer.innerHTML = `
        <div class="relative overflow-hidden rounded shadow-sm bg-white border border-gray-200 h-96">
          <div id="ads-slider-inner" class="flex flex-col transition-transform duration-700 ease-in-out w-full h-full">
            ${ads.map(ad => {
              let imgUrl = '';
              ...
              const linkUrl = ad.link || '#';
              return `
                <div class="w-full shrink-0 h-96">
                  <a href="${linkUrl}" target="_blank" rel="noopener noreferrer" class="block w-full h-full">
                    <img src="${imgUrl}" alt="${escapeHtml(ad.title)}" class="w-full h-full object-cover">
                  </a>
                </div>
              `;
            }).join('')}
      ...
      if (ads.length > 1) {
        ...
        setInterval(() => {
          ...
          const sliderInner = document.getElementById('ads-slider-inner');
          if (sliderInner) {
            sliderInner.style.transform = `translateY(-${currentAdIndex * 100}%)`;
          }
        }, 4000);
      }
    } else {
